...
class SeamDetector:
    ...
    def auto_detect_all_seams(
        ...
        secondary_surfaces = secondary_extractor.extract_all_surfaces(secondary_link)
        touching_pairs = self.analyzer.find_touching_pairs(main_surfaces, secondary_surfaces)
        ...
        if not touching_pairs:
            ...
            for i, m in enumerate(main_surfaces):
                for j, s in enumerate(secondary_surfaces):
                    ...
                    print(f"  Main[{i}] vs Sec[{j}]: dot={dot:.3f}, dist={dist*1000:.1f}mm")
                    if dot < -0.99:
                        print(f"    ^^ Normals face each other!")
                    if dist < 0.020:
                        print(f"    ^^ Distance OK!")
        ...
        for main_surface, secondary_surface in touching_pairs:
            try:
                overlap_surface = self.analyzer.get_overlap_surface(main_surface, secondary_surface)
            except ValueError as e:
                logger.warning(
                    f"Skipping surface pair (main: {main_surface.get('face_id', 'unknown')}, "
                    f"secondary: {secondary_surface.get('face_id', 'unknown')}): {e}"
                )
                continue

            # Extract raw seam lines
            seam_lines = self._extract_seam_lines_from_overlap(
                overlap_surface, main_surface, secondary_surface, min_seam_length_m
            )
            ...
            for start, end in seam_lines:
                ...
                for seg in segments:
                    if seg['length'] < min_seam_length_m or seg['on_surface'] == 'none':
                        continue

                    # Calculate reference surface, away_from_wall_vector, and edge joint flag
                    ref_surface, away_vec, is_edge_joint = self._calculate_segment_vectors(
                        seg['start'], seg['end'],
                        main_surface, secondary_surface, overlap_surface,
                        seg['on_surface']
                    )
    ...
    def _calculate_segment_vectors(
        ...
        Args:
            ...
            on_surface: Which surface segment is on ('main', 'secondary', 'both', 'none').

        Returns:
            Tuple of (reference_surface, away_from_wall_vector, is_edge_joint).
        """
        ...
        # Determine joint type and reference surface based on boundary positions
        if seam_on_main_boundary and seam_on_sec_boundary:
            # CASE 1: Edge-on-edge (seam lies on boundary edge of BOTH surfaces)
            ...
            if self.analyzer.is_point_inside_surface(test_point, secondary_surface, tolerance_m=0.01):
                away_vec = -perpendicular
            else:
                away_vec = perpendicular

        elif seam_on_main_boundary and not seam_on_sec_boundary:
            # CASE 2: Main's boundary edge lies on secondary's surface (flat joint)
            ...
            # CASE 3: Secondary's boundary edge lies on main's surface (flat joint)
            # Secondary's edge is on top of main's surface
            is_edge_joint = False
            reference_surface = secondary_surface
            # Point away from main
            test_point = (seg_start + seg_end) / 2.0 + perpendicular * 0.01
            if self.analyzer.is_point_inside_surface(test_point, main_surface, tolerance_m=0.01):
                away_vec = -perpendicular
            else:
                away_vec = perpendicular

        else:
            # CASE 4: Internal segment (neither lies on boundaries, use overlap)
            # This shouldn't happen often - might be internal weld or detection artifact
            is_edge_joint = False
            reference_surface = overlap_surface
    ...
    def _is_seam_on_surface_boundary(
        ...
        The seam must lie along a boundary edge, not just have endpoints near boundaries.
        ...
        Returns:
            True if the entire seam line lies on a boundary edge of the surface.
        """
        # Project both endpoints to surface UV coordinates
        u_start, v_start = self.analyzer.project_point_to_surface(seg_start, surface)
        u_end, v_end = self.analyzer.project_point_to_surface(seg_end, surface)
        
        half_u = surface['bounds'][0] / 2.0
        half_v = surface['bounds'][1] / 2.0
        
        # Check if line lies along U boundary (left or right edge)
        # Both points must be at same U boundary and have consistent U coordinate
        on_left_or_right_edge = (
            (abs(abs(u_start) - half_u) < tolerance_m) and
            (abs(abs(u_end) - half_u) < tolerance_m) and
            (abs(u_start - u_end) < tolerance_m)  # Same U coordinate (parallel to V axis)
        )
        
        # Check if line lies along V boundary (top or bottom edge)
        # Both points must be at same V boundary and have consistent V coordinate
        on_top_or_bottom_edge = (
            (abs(abs(v_start) - half_v) < tolerance_m) and
            (abs(abs(v_end) - half_v) < tolerance_m) and
            (abs(v_start - v_end) < tolerance_m)  # Same V coordinate (parallel to U axis)
        )
    ...
    def _split_seam_at_boundaries(
        ...
        Returns:
            List of segment dicts with 'start', 'end', 'on_surface', 'length'.
        """
        main_corners = self.analyzer.get_surface_corners(main_surface)
        secondary_corners = self.analyzer.get_surface_corners(secondary_surface)

        main_edges = [(main_corners[i], main_corners[(i+1)%4]) for i in range(4)]
        secondary_edges = [(secondary_corners[i], secondary_corners[(i+1)%4]) for i in range(4)]
        ...
        for edge_start, edge_end in main_edges + secondary_edges:
            ...
            if intersection_2d is not None:
                t = np.linalg.norm(intersection_2d - seam_start_2d) / (np.linalg.norm(seam_end_2d - seam_start_2d) + 1e-10)
                intersection_3d = seam_start + t * (seam_end - seam_start)
                intersections.append(intersection_3d)

        if not intersections:
            on_main = self.analyzer.is_point_inside_surface((seam_start + seam_end) / 2, main_surface, tolerance_m=0.001)
            on_secondary = self.analyzer.is_point_inside_surface((seam_start + seam_end) / 2, secondary_surface, tolerance_m=0.001)
        ...
        for i in range(len(points) - 1):
            ...
            on_main = self.analyzer.is_point_inside_surface(seg_midpoint, main_surface, tolerance_m=0.001)
            on_secondary = self.analyzer.is_point_inside_surface(seg_midpoint, secondary_surface, tolerance_m=0.001)
    ...
    def _line_segment_intersection_2d(
        p1: NDArray, p2: NDArray, p3: NDArray, p4: NDArray, tolerance: float = 1e-10
    ) -> Optional[NDArray]:
    ...
    def _extract_seam_lines_from_overlap(
        ...
        edges = [(overlap_corners[i], overlap_corners[(i + 1) % 4]) for i in range(4)]
        seam_lines = []
        ...
        for edge_start, edge_end in edges:
            ...
            start_in_main = self.analyzer.is_point_inside_surface(edge_start, main_surface, tolerance_m=0.001)
            end_in_main = self.analyzer.is_point_inside_surface(edge_end, main_surface, tolerance_m=0.001)
            overlaps_main = start_in_main or end_in_main

            start_in_secondary = self.analyzer.is_point_inside_surface(edge_start, secondary_surface, tolerance_m=0.001)
            end_in_secondary = self.analyzer.is_point_inside_surface(edge_end, secondary_surface, tolerance_m=0.001)
    ...
    def _is_edge_at_surface_boundary(
        ...
        u_start, v_start = self.analyzer.project_point_to_surface(edge_start, surface)
        u_end, v_end = self.analyzer.project_point_to_surface(edge_end, surface)
